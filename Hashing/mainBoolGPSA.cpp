...
position wset[800];
bool dlist[20000] = {false};
int wset1 = 0, wset2 = 0, dlist1 = 0;
...
void single_step() {
	...
	void add_to_wset(position p);
	void add_to_dlist(int p);
	void rotation(int b[3][3]);
	void reflection(int b[3][3]);
	int is_duplicate(int p[3][3], int q[3][3]);
	position get_from_wset();
	bool search_dlist(int p);
	void print_status();
	int min_boardval(position p);
	game is_win_or_draw(position p, int xo);
...
//	check for duplicates
				if (!search_dlist(min_boardval(possible))) {	// not found
...
void add_to_dlist(int minboardval){
	if (dlist[minboardval] == false) {
		dlist[minboardval] = true;
		dlist1++;
	}
}
