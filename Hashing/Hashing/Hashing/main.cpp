...
std::map<int,bool>hashTableMap;		//  Use a Map for the duplicates list to cut down on 
...
void main() {
	...
	void print_status(double t);
	void add_to_wset(position p);
	int wset_not_empty();
	void add_to_dlist(int minboardval);
	void printp(position p);
	void printg(game gg);
	int min_boardval(position k);
	...
	//and initialise wset and HashTable
	add_to_wset(temp);
	add_to_dlist(min_boardval(temp));
...
void single_step() {
	...
	bool search_dlist(int minboardval);
	void print_status();
	game is_win_or_draw(position p, int xo);
...
//	check for duplicates
				if (search_dlist(min_boardval(possible))==false) {	// not found
//	add survivors to wset
					// first set g field
					possible.g = is_win_or_draw(possible, xo);	
					add_to_wset(possible);
//	add all 8 equivalents to hashTable. Yes. "if not already present" - No
...
/*	This is really crude, some positions will be added multiple times
	...
	For now I'll just leave this - it is simple at least!			*/
					...
					for(k=0; k<4; k++) {
						...
						add_to_dlist(min_boardval(possible));
					}
					// Now the mirror images
...
bool search_dlist(int minboardval) {
	return hashTableMap.find(minboardval)!=hashTableMap.end();
}
...
void print_status(double t){
	cout << endl << "Working set size     = " << wset2;
	cout << "\tDuplicates list size = " << hashTableMap.size() << endl;
	cout << "Time to Run = " << t << "s" << endl << endl;
}
